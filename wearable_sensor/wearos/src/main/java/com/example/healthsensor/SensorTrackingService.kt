 ...
 *   /sensor/ecg         — 심전도 (온디맨드)
 *   /sensor/spo2        — 혈중 산소포화도 (온디맨드)
 *   /sensor/bia_fat     — 체지방률
 *   /sensor/bia_bmr     — 기초대사량
 *   /sensor/bia_muscle  — 골격근량
...
class SensorTrackingService : Service() {
    ...
    private val ecgBuf       = mutableListOf<Pair<Long, Float>>()
    private val spo2Buf      = mutableListOf<Pair<Long, Float>>()
    private val biaFatBuf    = mutableListOf<Pair<Long, Float>>()
    private val biaBmiBuf    = mutableListOf<Pair<Long, Float>>()
    private val biaMuscleBuf = mutableListOf<Pair<Long, Float>>()
    private val biaWaterBuf  = mutableListOf<Pair<Long, Float>>()
    private val sweatLossBuf = mutableListOf<Pair<Long, Float>>()
    ...
    private fun startTracking() {
        ...
        serviceScope.launch {
            sensorManager.ecgFlow.collect { d ->
                ecgBuf.add(d.timestamp to d.ecg)
                SensorState.ecgText.value = "%.2f mV".format(d.ecg)
            }
        }
        serviceScope.launch {
            sensorManager.spo2Flow.collect { d ->
                spo2Buf.add(d.timestamp to d.spo2.toFloat())
                SensorState.spo2Text.value = "${d.spo2}%"
            }
        }
        serviceScope.launch {
            sensorManager.biaFlow.collect { d ->
                // BIA 4개 지표를 별도 채널로 각각 전송
                biaFatBuf.add(d.timestamp to d.bodyFat)
                biaBmiBuf.add(d.timestamp to d.bmr)
                biaMuscleBuf.add(d.timestamp to d.skeletalMuscleMass)
                biaWaterBuf.add(d.timestamp to d.bodyWaterMass)
                SensorState.biaText.value = "지방 %.1f%% / BMR %.0f".format(d.bodyFat, d.bmr)
            }
        }
        serviceScope.launch {
            sensorManager.sweatLossFlow.collect { d ->
                sweatLossBuf.add(d.timestamp to d.sweatLoss)
                SensorState.sweatLossText.value = "%.0f ml".format(d.sweatLoss)
            }
        }
        ...
        serviceScope.launch {
            while (true) {
                ...
                if (ecgBuf.isNotEmpty())       phoneSender.sendBatch("/sensor/ecg",        ecgBuf.toList().also       { ecgBuf.clear() })
                if (spo2Buf.isNotEmpty())      phoneSender.sendBatch("/sensor/spo2",       spo2Buf.toList().also      { spo2Buf.clear() })
                if (biaFatBuf.isNotEmpty())    phoneSender.sendBatch("/sensor/bia_fat",    biaFatBuf.toList().also    { biaFatBuf.clear() })
                if (biaBmiBuf.isNotEmpty())    phoneSender.sendBatch("/sensor/bia_bmr",    biaBmiBuf.toList().also    { biaBmiBuf.clear() })
                if (biaMuscleBuf.isNotEmpty()) phoneSender.sendBatch("/sensor/bia_muscle", biaMuscleBuf.toList().also { biaMuscleBuf.clear() })
                if (biaWaterBuf.isNotEmpty())  phoneSender.sendBatch("/sensor/bia_water",  biaWaterBuf.toList().also  { biaWaterBuf.clear() })
                if (sweatLossBuf.isNotEmpty()) phoneSender.sendBatch("/sensor/sweat_loss", sweatLossBuf.toList().also { sweatLossBuf.clear() })
            }
        }
    ...
    private fun stopAndClean() {
        ...
        SensorState.ecgText.value      = "—"
        SensorState.spo2Text.value     = "—"
        SensorState.biaText.value      = "—"
        SensorState.sweatLossText.value = "—"
    ...
    private fun buildNotification(): Notification {
        ...
        return Notification.Builder(this, channelId)
            ...
            .setContentText("HR · PPG · EDA · Accel · SkinTemp · ECG · SpO2 · BIA · 발한량 수집 중")
