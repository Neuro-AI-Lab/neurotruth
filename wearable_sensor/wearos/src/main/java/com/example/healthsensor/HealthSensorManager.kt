 ...
 *   - ECG_ON_DEMAND              : 심전도 (온디맨드, Samsung 정책 승인 필요)
 *   - SPO2_ON_DEMAND             : 혈중 산소포화도 (온디맨드)
 *   - BIA_ON_DEMAND              : 체성분 (온디맨드)
 *   - SWEAT_LOSS                 : 발한량
 *
 * ※ 온디맨드 센서 제약:
...
class HealthSensorManager(private val context: Context) {
     ...
     * PPG/Accel은 SDK가 ~12초치를 버퍼링 후 전달하는데, 500ms마다 flush하면
     * 실시간에 가까운 수신이 가능하다.
     */
