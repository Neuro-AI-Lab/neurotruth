...
| D-019 | Login-day visibility | At successful demo login, create elapsed current-day Watch-worn periods before the recent-hour trace plus one current-day event, event-linked AUQ, completed dialogue session, inference, and report; use event-linked AUQ scores in the visible 39..41 range of the valid 0..48 scale. | user | The current day lacked a visible stacked history and the event/AUQ histories were still too sparse for recording. | The day stacked dashboard, event, and AUQ views are populated immediately at the login-relative current time without creating future rows or changing clinical interpretation. | confirmed | resolved |
