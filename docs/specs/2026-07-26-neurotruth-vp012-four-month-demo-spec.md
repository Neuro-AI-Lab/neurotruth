...
| D-018 | Historical dashboard consistency | Represent only deterministic morning, midday, and evening Watch-worn periods with weighted 15-minute rows. Leave all other hours unmeasured, distribute 29 historical event days across 120 days, and back every event with a real three-danger sequence. | user | The user rejected a calendar that looked continuously measured for all 24 hours and asked for more visible event history. | A measured hour still totals 360 ten-second samples, but daily totals remain below the 8,640 maximum and unmeasured hours stay empty. | confirmed | resolved |
| D-019 | Login-day visibility | At successful demo login, create the recent-hour trace plus one current-day event, event-linked AUQ, completed dialogue session, inference, and report; use event-linked AUQ scores in the visible 39..41 range of the valid 0..48 scale. | user | The current day had predictions but no event or AUQ, and both histories appeared too sparse for the recording. | The day dashboard is populated immediately at the login-relative current time without creating future rows or changing clinical interpretation. | confirmed | resolved |
...
- **FR-004:** Alerts shall be tied to danger predictions, at least 15 minutes apart, and labeled as demo records. The login day shall include one alert from the recent-hour trace.
