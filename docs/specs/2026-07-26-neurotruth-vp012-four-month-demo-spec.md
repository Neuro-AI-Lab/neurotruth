...
| D-018 | Historical dashboard consistency | Represent every complete historical hour with four weighted 15-minute rows, each equivalent to 90 ten-second samples; distribute 16 event days across the 120-day period, show a 90-minute closing-time danger block on each, and back every event with one three-danger sequence. | user | The previous seed stored only 24 rows per day against the dashboard's 8,640-sample daily scale, while high-stage days and eight clustered alert rows did not agree. | Complete historical days now total 8,640 samples, every hour totals 360, and event counts agree with visible danger-stage evidence without inserting over one million fictional rows. | confirmed | resolved |
