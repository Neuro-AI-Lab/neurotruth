...
from .stt_client import SttError
...
async def transcribe(
    ...
    language: Annotated[Literal["ko"], Form()] = "ko",
) -> dict[str, Any]:
    client = runtime.stt_client
    ...
    path: Path | None = None
    try:
        client.tmpfs_root.mkdir(parents=True, exist_ok=True)
        descriptor, name = tempfile.mkstemp(prefix="nt-stt-", suffix=suffix, dir=client.tmpfs_root)
        os.close(descriptor)
        path = Path(name)
        size = 0
        with path.open("wb") as handle:
            while chunk := await audio.read(1024 * 1024):
                size += len(chunk)
                if size > client.max_upload_bytes:
                    raise HTTPException(status_code=413, detail="audio_too_large")
                handle.write(chunk)
        if size == 0:
            raise HTTPException(status_code=422, detail="no_speech")
        try:
            return await client.transcribe(path, content_type=audio.content_type)
