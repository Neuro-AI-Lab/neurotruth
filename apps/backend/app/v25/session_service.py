...
from .dashboard_service import class_from_schema
...
class SessionService:
    def __init__(self, repository: V25Repository, keyring: AesGcmKeyring,
                 auth_service: Any, agent: BedrockSessionAgent | Any) -> None:
        self.repository, self.keyring, self.auth_service, self.agent = repository, keyring, auth_service, agent
    ...
    async def _create_inference(self, patient_id: UUID, session_id: UUID, scope: str,
        ...
        if not summarize or not await self._ai_allowed(patient_id):
            summary_status = "unavailable"
        else:
            try:
                model_id = await self._model("state_inference_agent", STATE_PROMPT_VERSION)
                summary = await self.agent.summarize_state(payload)
    ...
    def _public_inference(self, patient_id: UUID, row: dict[str, Any] | None) -> dict[str, Any] | None:
        ...
        )).decode("utf-8") if row.get("summary_encrypted") else None
        return {"inferenceId": str(row["id"]), "scope": row["inference_scope"], "state": row["state_class"],
                "confidence": float(row["confidence"]) if row.get("confidence") is not None else None,
                "summaryStatus": row["summary_status"], "summary": summary,
                "createdAt": row["created_at"].isoformat()}
    ...
    async def _queue_report(self, patient_id: UUID, session_id: UUID, partial: bool) -> dict[str, Any]:
        model = await self._model("report_agent", "intervention-report-v1")
    ...
    async def _run_report(self, patient_id: UUID, session_id: UUID, report_id: UUID, partial: bool) -> None:
        try:
            ...
            evidence = await self.repository.inference_evidence(patient_id, session_id)
            content = await self.agent.report(evidence=self._json_safe(evidence), history=await self._history(patient_id, session_id), partial=partial)
    ...
    async def _model(self, component: str, prompt_version: str) -> UUID:
        return await self.repository.ensure_agent_model_version(
            component=component, model_name=self.agent.model_name, model_version=self.agent.model_version,
