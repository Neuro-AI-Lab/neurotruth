...
from .dependencies import get_runtime, password_ready_user, patient_user
from .models import UserRecord
from .rppg_media import InvalidRppgMedia
from .rppg_repository import RppgConflict
from .rppg_service import RppgConsentRequired, RppgJobNotFound, RppgService, RppgUnavailable
from .runtime import V25Runtime
from .routes_admin import admin_user


router = APIRouter()


def get_rppg_service(runtime: Annotated[V25Runtime, Depends(get_runtime)]) -> RppgService:
...
async def retry_job(job_id: UUID, runtime: Annotated[V25Runtime, Depends(get_runtime)],
                    user: Annotated[UserRecord, Depends(patient_user)],
                    service: Annotated[RppgService, Depends(get_rppg_service)]) -> dict[str, Any]:
    consent = await runtime.repository.current_consent(user.id)
...
class RevealVideoBody(BaseModel):
    ...
    def reason_required(cls, value: str) -> str:
        if not value.strip(): raise ValueError("Reason is required")
        return value.strip()
...
async def admin_captures(runtime: Annotated[V25Runtime, Depends(get_runtime)],
