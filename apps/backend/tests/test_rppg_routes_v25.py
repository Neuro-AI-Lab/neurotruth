...
class FakeRppgService:
    ...
    def __init__(self, root: Path):
        ...
        self.inspector = SimpleNamespace(inspect=lambda path: VideoInspection(10.0, 30.0, 1280, 720))
        self.accepted = None
...
def app_client(workdir: Path, *, all_consents: bool = True, max_mib: int = 20):
...
def test_upload_contract_returns_202_and_cleans_plaintext(workdir: Path) -> None:
    ...
    response = client.post("/api/rppg/jobs", data={
        "clientCaptureId": str(capture_id), "capturedAtMs": "1800000000000", "durationMs": "10000",
