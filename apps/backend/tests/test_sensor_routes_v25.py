...
from app.v25.models import SensorResultRecord
from app.v25.repository import RepositoryConflictError
from app.v25.routes_sensor import SensorWindow, get_sensor_service, router
from app.v25.dependencies import get_runtime, patient_user
from app.v25.sensor_service import SensorModelUnavailable, SensorPayloadConflict, SensorService
...
def payload(window_id: UUID | None = None, *, value: float = 1.2) -> dict[str, Any]:
    return {
        ...
        "sequence": 1, "sentAtMs": 1_700_000_010_100,
        "windowStartMs": 1_700_000_000_000, "windowEndMs": 1_700_000_010_000,
        "windowMs": 10_000,
...
def make_service(directory: str) -> tuple[SensorService, FakeRepository, FakePredictor]:
...
def test_notification_consent_off_persists_and_publishes_prediction_without_alert() -> None:
    async def scenario() -> None:
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            ...
            assert (await asyncio.wait_for(queue.get(), timeout=0.1))["predictionId"] == result["predictionId"]
            await service.hub.unsubscribe(patient_id, queue)
...
def test_prediction_hub_never_crosses_patient_queues() -> None:
    async def scenario() -> None:
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            service, _, _ = make_service(directory)
