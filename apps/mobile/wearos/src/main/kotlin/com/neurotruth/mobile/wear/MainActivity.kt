 ...
 * The watch UI.
 *
 * It requests the sensor permissions and starts or stops [SensorTrackingService]. Everything shown
 * comes from [SensorState], so the screen updates without the activity talking to the service.
 *
 * There is no login here and no backend call anywhere in this module — the watch holds no
 * credentials.
 */
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            WatchScreen(
                ...
                onToggle = {
                    when {
                        isTracking -> SensorTrackingService.stop(this)
                        hasSensorPermissions() -> SensorTrackingService.start(this)
                        else -> requestPermissions(SENSOR_PERMISSIONS, PERMISSION_REQUEST)
                    }
                },
            )
        }
    ...
    override fun onRequestPermissionsResult(
        ...
        val sensorPermissionDenied = permissions.zip(grantResults.toTypedArray())
            .any { (permission, result) ->
                permission in SENSOR_PERMISSIONS && result != PackageManager.PERMISSION_GRANTED
            }
        if (requestCode == PERMISSION_REQUEST && sensorPermissionDenied) {
            SensorState.status.value = "권한 거부됨 · 설정에서 허용해 주세요"
        }
    }
        ...
        Box(
            ...
            Column(
                ...
                Row(
                    ...
                    Text(
                        text = if (isTracking) "측정 중" else "대기",
                        fontSize = 9.sp,
                        color = if (isTracking) Color(0xFF10B981) else Color(0xFF7E8A95),
                    ...
                    Box(
                        modifier = Modifier
                            .size(7.dp)
                            .background(
                                if (isTracking) Color(0xFF10B981) else Color(0xFF7E8A95),
                                CircleShape,
                            ),
                    )
                }
                ...
                Box(
                    ...
                    Column(
                        ...
                        Box(
                            ...
                            Text(
                                text = watchStateBadge(cravingClass),
                                fontSize = 18.sp,
                                color = cravingColor,
                                fontWeight = FontWeight.Bold,
                            )
...
private fun watchStateBadge(cravingClass: Int?): String = when (cravingClass) {
    0 -> "낮음"
    1 -> "높음"
    else -> "대기"
}
